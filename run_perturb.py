...
from run_tokig import dump_token_interp_info, vis_analyze
from expl_methods.latattr_models import LAtAttrRobertaForQuestionAnswering
from expl_methods.perturb_models import run_lime_attribution, run_shap_attribution
...
def fit_locality(args, tokenizer, model, inputs, feature):    
    ...
    if args.variant == 'lime':
        np_attribution = run_lime_attribution(args, len(tokens), score_fn).reshape((1,-1))
...
def predict_and_fit_locality(args, batch, model, tokenizer, batch_features, batch_examples):
    model.eval()
    ...
    with torch.no_grad():
        importances = fit_locality(args, tokenizer, model, inputs, batch_features[0])
